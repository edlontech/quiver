...
class HttpServerProtocol(QuicConnectionProtocol):
    ...
    def _h3_event_received(self, event: H3Event) -> None:
        ...
        if isinstance(event, HeadersReceived):
            ...
            self._handle_datagram(event.stream_id, event.data)

    def _handle_datagram(self, stream_id: int, data: bytes) -> None:
    ...
    def _handle_headers(
        ...
        if end_stream and request.method in ("GET", "HEAD"):
            ...
            # Extended CONNECT keeps the stream open; respond on headers.
            self._send_response(stream_id)
    ...
    def _send_response(self, stream_id: int) -> None:
        ...
        elif path == "/datagrams/echo" and method == "CONNECT":
            self._http.send_headers(stream_id, [(b":status", b"200")], end_stream=False)
            self.transmit()
            # Do NOT call _send_simple_response and do NOT pop self._requests[stream_id]
            # — keep the request alive so datagrams keep flowing on this stream.
            return
        elif method == "HEAD":
